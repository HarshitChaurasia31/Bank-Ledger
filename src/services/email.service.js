...
transporter.verify((error,success)=>{
    if(error){
        console.error('Error connecting to gmail server:',error)
    }
    else{
...
async function sendTransactionFailureEmail(userEmail,name,amount,toAccount) {
  ...
  const html = `<p>Hello ${name},</p><p>We regret to inform you that a transaction of amount <strong>${amount}</strong> to account <strong>${toAccount}</strong> has failed.</p><p>Please check your account and try again.</p><p>Best regards,<br>The Backend Ledger Team</p>`;
  await sendEmail(userEmail, subject, text, html);
}
